...
//  dentakuApp
//
//  Created by Nirai on 2021/11/07.
//
...
struct ContentView: View {
        ...
        ZStack {
            ...
            VStack {
                ...
                HStack {
                    ...
                    Text(selectedItem == "0" ? checkfloat(number: caluculatedNumber) : selectedItem)
    ...
    // 小数点以下に数値が含まれるか確認する
    private func checkfloat(number: Double) -> String {
...
struct NumberView: View {
    ...
    private func handleButtonInfo(item: String) {
        ...
        // 数字が入力された時
        if numbers.contains(item) {
            // "."入力されて、且つ入力済みの値に"."が含まれる時に入力しない
            if item == "." && selectedItem.contains(".") {
                return
            }

            if item == "0" && selectedItem.contains(".") {
                handleButtonInfo(item: selectedItem)
            }
